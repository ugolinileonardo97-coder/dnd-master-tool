...
export function MobileBase44App(props) {
  ...
  function renderHome() {
    const cards = [
      ["merchants", "merchants", "Mercanti", "Genera e gestisci"],
      ["taverns", "taverns", "Locande", "Ospitalita e quest"],
      ["bestiary", "bestiary", "Bestiario", "Consulta creature"],
      ["combat", "combat", "Combattimento", "Gestisci scontri"],
      ["map", "map", "Mappa", "Organizza per zona"],
      ["settings", "settings", "Impostazioni", "Partite e opzioni"],
    ];
        ...
        <section className="b44-section-grid">
          {cards.map(([id, icon, label, desc]) => (
            <button key={id} type="button" className="b44-section-card" onClick={() => setScreen(id)}>
              <span><FantasyIcon name={icon} decorative /></span>
              <strong>{label}</strong>
              <small>{desc}</small>
            </button>
          ))}
